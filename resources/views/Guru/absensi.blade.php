...
<body class="bg-gray-100 font-sans antialiased">
    ...
    <main class="lg:ml-64 min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 p-4 md:p-6 lg:p-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
            <div><h2 class="text-3xl font-bold text-gray-800">📋 Absensi Kelas</h2><p class="text-gray-600 mt-1">{{ $kelas ? $kelas->nama_kelas : 'Anda belum memiliki kelas' }}</p></div>
            <div class="mt-2 text-sm text-gray-500">Tanggal: {{ date('d F Y') }}</div>
        </div>

        @if(!$kelas)
            <div class="bg-white rounded-2xl shadow-md p-8 text-center"><i class="fas fa-chalkboard-teacher text-6xl text-gray-300 mb-4"></i><p class="text-gray-500">Anda belum ditugaskan sebagai wali kelas.</p></div>
        @else
            <div class="bg-white rounded-2xl shadow-md overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50"><tr><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">#</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Nama Murid</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">NIS</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Status</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Waktu Absen</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Aksi</th></tr></thead>
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap"><span class="status-badge status-{{ $murid->status }}">{{ ucfirst($murid->status) }}</span></td>
                            ...
                            @empty
                            <tr><td colspan="6" class="px-6 py-4 text-center text-gray-500">Belum ada murid di kelas ini.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
    ...
    <script>
        ...
        function updateStatus(userId, status) {
            ...
            }).then(res => res.json()).then(data => { if (data.success) location.reload(); else alert('Gagal update status'); });
        }
    </script>
</body>
</html>